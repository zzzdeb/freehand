...
DIR_RAW = Path(os.path.expanduser("~"), "workspace/forearm_US")
NUM_SCANS = 12
RESAMPLE_FACTOR = 4
PATH_SAVE = Path(os.path.expanduser("~"), "workspace/")
DELAY_TFORM = 4  # delayed tform from temporal calibration
fh5_path = PATH_SAVE / 'frames_res{}.h5'.format(RESAMPLE_FACTOR)
# if fh5_path.is_file():
#     os.remove(fh5_path)

fh5_frames = h5py.File(fh5_path,'a') 
if FLAG_SCANFILE:
    fh5_scans = h5py.File(PATH_SAVE/ 'scans_res{}.h5'.format(RESAMPLE_FACTOR),'a')
...
for i_sub, folder in enumerate(folders_subject[:]):
    ...
    scan_crop_idx = pd.read_csv(DIR_RAW/folder/fn_csv[0], index_col=0)
    ...
    for i_scan, fn in enumerate(fn_mha):
        ...
        frames, tforms, _ = read_frame_transform(
            ...
            scan_crop_indices = list(scan_crop_idx.loc[fn]),
